...
## 12.10.1 Ollama 一键配置与 OpenCode Provider

> **📦 v2026.3.11 新增**

**Ollama 一键 Onboarding：**

Onboarding 向导新增 Ollama 一键配置支持。用户在 `openclaw onboard` 流程中选择 Ollama 后，系统自动检测本地 Ollama 服务、拉取推荐模型、配置 provider。支持两种模式：

- **Local**：所有请求走本地 Ollama，完全离线可用
- **Cloud+Local 混合**：主模型使用云端（如 Anthropic），fallback 和辅助任务（embedding、轻量推理）使用本地 Ollama

**OpenCode Go Provider：**

新增 OpenCode Go 语言 provider，与 Zen 共享同一个 key 存储。如果用户已配置 Zen 的 API key，OpenCode provider 自动可用，无需额外配置。

---

## 12.10.2 Fast Mode 与 Provider Plugin 架构

> **📦 v2026.3.12 新增**

**Fast Mode 支持：**

新增共享的 `/fast` 开关，允许用户在对话中一键切换到快速模式。Fast Mode 的核心配置项是 `params.fastMode`：
...
Fast Mode 在不同 provider 的映射行为：
- **OpenAI**：映射到 `service_tier` 参数（使用更快的推理层）
- **Anthropic**：直接透传到 API（使用 fast 推理模式）
- 支持实时验证：切换 Fast Mode 后立即检查 provider 是否支持，不支持则回退并通知用户

**Provider Plugin 架构迁移：**

Ollama、vLLM、SGLang 三个本地 provider 从 core 内置代码迁移至 **Provider Plugin 架构**。每个 provider 现在是一个独立的 plugin，自管理以下流程：
- **Onboarding**：引导用户完成安装和配置
- **Model Discovery**：动态发现可用模型列表
- **Model-Picker Setup**：配置模型选择器的选项和默认值
- **Post-Selection Hooks**：用户选择模型后的后置操作（如下载模型、验证连接）

这一架构变化使得添加新的本地 provider 不再需要修改核心代码（另见第 21 章 Plugin SDK 的相关更新）。
