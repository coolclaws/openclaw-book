...
防止 Agent 一次性读取多个 Skills 导致 context 被快速消耗。

---

## 12.6 always: true 的 Skills
...
对高频、低复杂度的操作（查天气、查汇率）有显著的延迟和成本优势。
...
2. **依赖工具**：`requires.bins`（ALL）/ `requires.anyBins`（ANY）是否存在
...
- `disableModelInvocation` 保护高权限 Skills 不被 Agent 自主触发
- 资格检查确保 Agent 不会尝试使用不满足依赖条件的 Skill
