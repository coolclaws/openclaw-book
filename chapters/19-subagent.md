...
## 19.11.1 sessions_yield 工具

> **📦 v2026.3.12 新增**

v2026.3.12 新增 `sessions_yield` 工具，允许编排器（orchestrator）**主动结束当前 turn**，跳过剩余排队的 tool work，并可携带隐藏的 follow-up payload 进入下一个 session turn。

```typescript
sessions_yield({
  reason?: string;       // 结束当前 turn 的原因（日志用）
  followUp?: {
    payload: string;     // 隐藏的 follow-up 内容（不显示给用户）
    delay?: number;      // 延迟多少毫秒后注入下一个 turn
  };
});
```
...
1. **编排器协调多个子 Agent**：编排器 spawn 了多个子 Agent 后，调用 `sessions_yield` 主动结束当前 turn，等待子 Agent 的 announce 推送。follow-up payload 可以携带"当所有子 Agent 完成后，执行汇总"的指令。

2. **避免 token 浪费**：当编排器在当前 turn 中已完成所有必要操作（spawn、steer），剩余的 tool work 队列中可能还有冗余任务。`sessions_yield` 立刻终止，不再消耗 token。

3. **跨 turn 状态传递**：follow-up payload 在下一个 turn 的 system prompt 中以隐藏消息形式注入，Agent 能读到但用户看不到，适合传递内部协调信息。
